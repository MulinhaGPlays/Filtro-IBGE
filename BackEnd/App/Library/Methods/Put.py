...
def Put(request, id, model, serializer, modelFieldID:str|None):
    model_data = JSONParser().parse(request)
    Model = get_object_or_404(model, pk=model_data[modelFieldID]) if id is None else get_object_or_404(model, pk=id)
